...
- `cmd/worker`: background worker process shell for async jobs.
- `cmd/realtime-dispatcher`: Kafka-to-Centrifugo dispatcher process shell.
- `internal/domain/voice`: domain types and event names.
- `internal/app/voiceapp`: application commands and business rules.
- `internal/infra/*`: adapters for memory, MongoDB, Kafka, LiveKit, Centrifugo, and media playback URLs.
