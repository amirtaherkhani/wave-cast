...
function SelectLike({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <button
      type="button"
      data-slot="select-like"
      className={cn(
        "flex h-10 items-center justify-between gap-2 rounded-3xl border border-border bg-background px-3 text-sm text-foreground transition-[color,box-shadow,background-color] outline-none hover:bg-muted focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/30",
        className,
      )}
    >
